...
#define APU_SEQ_CLOCK_DIVIDER     8  /* seq clock is 6000  */
#define APU_LFO_CLOCK_DIVIDER   128  /* lfo clock is 375   */
#define APU_VCA_CLOCK_DIVIDER    16  /* vca clock is 3000  */
#define APU_PCM_CLOCK_DIVIDER     2  /* pcm clock is 24000 */

#define APU_TIMER_CLOCK_DIVIDER 128  /* lcm of the other dividers */
...
/* phase tables */
#define APU_VCA_PHASE_TABLE_SIZE 144
#define APU_VCA_PHASE_MAX_INDEX  143 /* table_size - 1 */

static unsigned short S_apu_vca_phase_incs_table[APU_VCA_PHASE_TABLE_SIZE] = 
  {    14,    15,    16,    17,    18,    19,    20,    21,
       23,    24,    25,    27,    28,    30,    32,    34,
       36,    38,    40,    43,    45,    48,    51,    54,
       57,    60,    64,    68,    72,    76,    80,    85,
       90,    96,   101,   107,   114,   121,   128,   135,
      143,   152,   161,   170,   181,   191,   203,   215,
      228,   241,   255,   271,   287,   304,   322,   341,
      361,   383,   405,   430,   455,   482,   511,   541,
      573,   608,   644,   682,   722,   765,   811,   859,
      910,   964,  1022,  1082,  1147,  1215,  1287,  1364,
     1445,  1531,  1622,  1718,  1820,  1929,  2043,  2165,
     2294,  2430,  2574,  2728,  2890,  3062,  3244,  3437,
     3641,  3857,  4087,  4330,  4587,  4860,  5149,  5455,
     5780,  6123,  6487,  6873,  7282,  7715,  8174,  8660,
     9174,  9720, 10298, 10910, 11559, 12246, 12975, 13746,
    14564, 15430, 16347, 17319, 18349, 19440, 20596, 21821,
    23118, 24493, 25949, 27492, 29127, 30859, 32694, 34638,
    36698, 38880, 41192, 43641, 46236, 48986, 51899, 54985
  };

/* rise/fall wavetables (8 bits) */
static unsigned short S_apu_vca_rise_curve_table[256] = 
  { 4095, 3967, 3843, 3723, 3607, 3494, 3385, 3279,
    3176, 3077, 2981, 2888, 2798, 2710, 2626, 2543,
    2464, 2387, 2312, 2240, 2170, 2102, 2037, 1973,
    1911, 1852, 1794, 1738, 1683, 1631, 1580, 1530,
    1483, 1436, 1391, 1348, 1306, 1265, 1225, 1187,
    1150, 1114, 1079, 1046, 1013,  981,  951,  921,
     892,  864,  837,  811,  786,  761,  737,  714,
     692,  670,  649,  629,  609,  590,  572,  554,
     537,  520,  504,  488,  473,  458,  444,  430,
     416,  403,  391,  379,  367,  355,  344,  333,
     323,  313,  303,  294,  284,  276,  267,  259,
     251,  243,  235,  228,  221,  214,  207,  201,
     194,  188,  182,  177,  171,  166,  161,  156,
     151,  146,  141,  137,  133,  129,  125,  121,
     117,  113,  110,  106,  103,  100,   97,   94,
      91,   88,   85,   82,   80,   77,   75,   73,
      70,   68,   66,   64,   62,   60,   58,   56,
      55,   53,   51,   50,   48,   47,   45,   44,
...
static unsigned short S_apu_vca_fall_curve_table[256] = 
  {    0,   16,   32,   48,   64,   80,   96,  112,
     ...
     385,  401,  418,  434,  450,  466,  482,  498,
     514,  530,  546,  562,  578,  594,  610,  626,
     642,  658,  674,  691,  707,  723,  739,  755,
     771,  787,  803,  819,  835,  851,  867,  883,
     899,  915,  931,  947,  964,  980,  996, 1012,
    1028, 1044, 1060, 1076, 1092, 1108, 1124, 1140,
    1156, 1172, 1188, 1204, 1220, 1237, 1253, 1269,
    1285, 1301, 1317, 1333, 1349, 1365, 1381, 1397,
    1413, 1429, 1445, 1461, 1477, 1493, 1510, 1526,
    1542, 1558, 1574, 1590, 1606, 1622, 1638, 1654,
    1670, 1686, 1702, 1718, 1734, 1750, 1766, 1783,
    1799, 1815, 1831, 1847, 1863, 1879, 1895, 1911,
    1927, 1943, 1959, 1975, 1991, 2007, 2023, 2039,
    2056, 2072, 2088, 2104, 2120, 2136, 2152, 2168,
    2184, 2200, 2216, 2232, 2248, 2264, 2280, 2296,
    2312, 2329, 2345, 2361, 2377, 2393, 2409, 2425,
    2441, 2457, 2473, 2489, 2505, 2521, 2537, 2553,
    2569, 2585, 2602, 2618, 2634, 2650, 2666, 2682,
    2698, 2714, 2730, 2746, 2762, 2778, 2794, 2810,
    2826, 2842, 2858, 2875, 2891, 2907, 2923, 2939,
    2955, 2971, 2987, 3003, 3019, 3035, 3051, 3067,
    3083, 3099, 3115, 3131, 3148, 3164, 3180, 3196,
    3212, 3228, 3244, 3260, 3276, 3292, 3308, 3324,
    3340, 3356, 3372, 3388, 3404, 3421, 3437, 3453,
    3469, 3485, 3501, 3517, 3533, 3549, 3565, 3581,
    3597, 3613, 3629, 3645, 3661, 3677, 3694, 3710,
    3726, 3742, 3758, 3774, 3790, 3806, 3822, 3838,
    3854, 3870, 3886, 3902, 3918, 3934, 3950, 3967,
    3983, 3999, 4015, 4031, 4047, 4063, 4079, 4095
  };

/* rise/fall speed tables */
static unsigned short S_apu_vca_rise_speed_table[32] = 
  { 140, 137, 133, 129, 125, 122, 118, 114,
    110, 107, 103,  99,  95,  92,  88,  84,
     80,  77,  73,  69,  65,  62,  58,  54,
     50,  47,  43,  39,  35,  32,  28,  24
  };

static unsigned short S_apu_vca_fall_speed_table[32] = 
  { 116, 113, 109, 105, 101,  98,  94,  90,
     86,  83,  79,  75,  71,  68,  64,  60,
     56,  53,  49,  45,  41,  38,  34,  30,
     26,  23,  19,  15,  11,   8,   4,   0
  };

/* envelope keyscaling */
#define APU_ENV_KEYSCALING_DIVISOR  256
#define APU_ENV_TIME_KS_BREAKPOINT  (APU_NOTE_MIDDLE_C - 4 * 12 + 0)  /* C-0 */
#define APU_VCA_LEVEL_KS_BREAKPOINT (APU_NOTE_MIDDLE_C - 2 * 12 + 9)  /* A-2 */
...
#define APU_VCO_PHASE_TABLE_SIZE (12 * 64)

#define APU_VCO_PHASE_MAX_INDEX  6911 /* num_blocks * table_size - 1 */
...
enum
{
  ...
  APU_WAVE_REG_VCA_STAGE, 
  APU_WAVE_REG_VCA_PHASE, 
  APU_WAVE_REG_VCA_INDEX, 
  APU_WAVE_REG_VCA_LEVEL, 
  APU_WAVE_REG_VCO_PHASE, 
  APU_WAVE_REG_VCO_INDEX, 
  APU_WAVE_REG_VCO_LEVEL, 
...
int apu_reset()
{
  ...
  APU_PATCH_PARAM(0, ENV_AR) = 0;
  APU_PATCH_PARAM(0, ENV_DR) = 15;
  APU_PATCH_PARAM(0, ENV_SR) = 23;
  APU_PATCH_PARAM(0, ENV_RR) = 15;
...
int apu_play_note(unsigned short inst_num, unsigned short note)
{
  ...
  APU_WAVE_REG(inst_num, VCA_STAGE) = APU_VCA_STAGE_A;
  APU_WAVE_REG(inst_num, VCA_PHASE) = 0;
  APU_WAVE_REG(inst_num, VCA_INDEX) = 0;

  APU_WAVE_REG(inst_num, VCO_PHASE) = 0;
  APU_WAVE_REG(inst_num, VCO_INDEX) = 0;
...
int apu_advance_lfo()
{
  ...
  for (m = 0; m < APU_NUM_WAVE_VOICES; m++)
  {
    ...
    APU_WAVE_REG(m, LFO_INDEX) += (APU_WAVE_REG(m, LFO_PHASE) >> 15);

    APU_WAVE_REG(m, LFO_PHASE) &= 0x7FFF;
...
int apu_advance_vca()
{
  ...
  for (m = 0; m < APU_NUM_WAVE_VOICES; m++)
  {
    ...
    if (APU_WAVE_REG(m, VCA_STAGE) == APU_VCA_STAGE_A)
      rate = S_apu_vca_rise_speed_table[APU_PATCH_PARAM(patch_num, ENV_AR)];
    else if (APU_WAVE_REG(m, VCA_STAGE) == APU_VCA_STAGE_D)
      rate = S_apu_vca_fall_speed_table[APU_PATCH_PARAM(patch_num, ENV_DR)];
    else if (APU_WAVE_REG(m, VCA_STAGE) == APU_VCA_STAGE_S)
      rate = S_apu_vca_fall_speed_table[APU_PATCH_PARAM(patch_num, ENV_SR)];
    else
      rate = S_apu_vca_fall_speed_table[APU_PATCH_PARAM(patch_num, ENV_RR)];

    /* lookup phase increment */
    increment = S_apu_vca_phase_incs_table[rate];

    /* update phase (8.12 fixed point) */
    APU_WAVE_REG(m, VCA_PHASE) += increment;

    if (APU_WAVE_REG(m, VCA_PHASE) & 0xF000)
    {
      APU_WAVE_REG(m, VCA_INDEX) += (APU_WAVE_REG(m, VCA_PHASE) >> 12);

      if (APU_WAVE_REG(m, VCA_INDEX) & 0xFF00)
      {
        if (APU_WAVE_REG(m, VCA_STAGE) == APU_VCA_STAGE_A)
        {
    ...
    APU_WAVE_REG(m, VCA_PHASE) &= 0x0FFF;
    APU_WAVE_REG(m, VCA_INDEX) &= 0x00FF;
...
int apu_advance_vco()
{
  ...
  for (m = 0; m < APU_NUM_WAVE_VOICES; m++)
  {
    ...
    for (n = 0; n < S_apu_vco_phase_steps_table[block]; n++)
    {
      APU_WAVE_REG(m, VCO_PHASE) += increment;
      APU_WAVE_REG(m, VCO_INDEX) += (APU_WAVE_REG(m, VCO_PHASE) >> 10);
    ...
    APU_WAVE_REG(m, VCO_LEVEL) = final_level_db & 0x0FFF;

    if (APU_WAVE_REG(m, VCO_INDEX) >= 512)
      APU_WAVE_REG(m, VCO_LEVEL) |= 0x1000;
  }
...
int apu_advance_sample()
{
  ...
  for (m = 0; m < APU_NUM_WAVE_VOICES; m++)
  {
    ...
    if (APU_WAVE_REG(m, VCO_LEVEL) & 0x1000)
      samp_L -= voice_level;
    else
      samp_L += voice_level;
...
int apu_update()
{
  ...
  for (m = 0; m < APU_CLOCKS_PER_SAMPLE; m++)
  {
    ...
    if ((S_apu_timer % APU_VCA_CLOCK_DIVIDER) == 0)
      apu_advance_vca();
